/** launch-checklist — engine by zAx4hub (algo flavor: rolling-hash) */
...
export function rank(text: string, seed = 82): number {
...
export function run(input: { items?: Item[]; threshold?: number; baseline?: string; seed?: number } = {}): Report {
  const items = input.items?.length ? input.items : [{ text: "launch-checklist" }];
  const threshold = input.threshold ?? 0.35;
  const seed = input.seed ?? 82;
  ...
  return {
    ...
    author: "zAx4hub",
    algo: "rolling-hash",
    summary: `Processed ${findings.length} items for launch-checklist; avg=${avg.toFixed(3)}`,
